...
class OrderAPIController extends Controller
{
    ...
    private function stripPayment(Request $request)
    {
        ...
        /*NEW-CODE-START-88*/
        //Get restraunt lat long
        $restrauntDetails= DB::table('markets')
        ->select('latitude','longitude')
        ->where('id', $order->productOrders[0]->product->market->id)
        ->get();            


        //get restruant drivers
        $restrauntDrivers= DB::table('driver_markets')
        ->select('user_id')
        ->where('market_id', $order->productOrders[0]->product->market->id)
        ->get();


        $availableDrivers=[];

        foreach ($restrauntDrivers as $restrauntDriver){

         $restrauntDriversAvailable = DB::table('drivers')
         ->select('user_id','driver_lat','driver_long')
         ->where('user_id', $restrauntDriver->user_id)
         ->where('available', 1)
         ->whereNotNull('driver_lat')
         ->whereNotNull('driver_long')
         ->get();

         if(count($restrauntDriversAvailable) > 0){
           $driversAvailable = DB::table('orders')
           ->select('*')
           ->where('driver_id', $restrauntDriversAvailable['0']->user_id)
           ->where('active', 1)
           ->where('order_status_id','!=', 5)
           ->get();

           if(count($driversAvailable)==0){
            $availableDrivers[] = $restrauntDriversAvailable['0'];
          }
        }
      }
      ...
      if(count($availableDrivers) > 0){
       $time = Carbon::now();
       $table = 'temp_driver_location_table_cash'.$order->productOrders[0]->product->market->id.$time->hour.$time->minute.$time->second;
       DB::insert( DB::raw("CREATE TEMPORARY TABLE ".$table."(user_id VARCHAR(255),driver_lat VARCHAR(255),driver_long VARCHAR(255))"));

       foreach ($availableDrivers as $availableDriver){

        DB::insert( DB::raw("INSERT INTO ".$table."(user_id,driver_lat,driver_long) VALUES ('".$availableDriver->user_id."','".$availableDriver->driver_lat."','".$availableDriver->driver_long."')"));
      }

      $extractedData = DB::table("$table")
      ->select("user_id", DB::raw("6371 * acos(cos(radians(" . $restrauntDetails['0']->latitude . "))
                                        * cos(radians(driver_lat)) * cos(radians(driver_long) - radians(" . $restrauntDetails['0']->longitude . "))
        + sin(radians(" .$restrauntDetails['0']->latitude. ")) * sin(radians(driver_lat))) AS distance"))
      ->orderBy('distance', 'asc')
      ->first();

      $assignDriver = DB::table('orders')
      ->where('id', $order->id)
      ->update(['driver_id'=>$extractedData->user_id,'updated_at'=>Carbon::now()->toDateTimeString()]);
      if($assignDriver){
        $driver = $this->userRepository->findWithoutFail($extractedData->user_id);
        if (!empty($driver)) {
          Notification::send([$driver], new AssignedOrder($order));
        }
      }
      //delete temperory table
      DB::unprepared(DB::raw("DROP TEMPORARY TABLE ".$table.""));
    }
    /*NEW-CODE-END-88*/
