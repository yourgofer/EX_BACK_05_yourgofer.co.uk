...
class UserAPIController extends Controller
{
    ...
    function loginWithPhone(Request $request)
    {
        try {
            ...
            $checkPhoneExist = DB::table('custom_field_values')
            ->select('customizable_id')
            ->where('value',$request->input('phone_number'))
            ->where('custom_field_id',4)
            ->first();
            
                if (Auth::loginUsingId($checkPhoneExist->customizable_id) ){
                    $user = auth()->user();
                    $user->device_token = $request->input('device_token', '');
                    $user->save();
                    return $this->sendResponse($user, 'User retrieved successfully');
            ...
            if ($request->has('device_token')) {
                ...
                foreach (getCustomFieldsValues($customFields, $request) as $value) {
                    ...
                    if($value['custom_field_id'] == 4){
                        $checkPhoneNumber = DB::table('custom_field_values')
                        ->where('custom_field_id',4)
